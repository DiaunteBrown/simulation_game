...
public class Player extends Character {
	...
	public void keyEvent(KeyEvent k, World world) {
		...
		if((k.getKeyCode()==KeyEvent.VK_UP || k.getKeyChar()=='w') && (position.getY()-move>=0)) {
			if(world.isEmpty(new Point((int)position.getX()/tileSize, (int)(position.getY()-move)/tileSize), new Point(Settings.getCharacterWidth(), test)))
			...
			if(world.isEmpty(new Point((int)position.getX()/tileSize, (int)(position.getY()+Settings.getCharacterHeight()+move)/tileSize), new Point(Settings.getCharacterWidth(), test)))
				setPosition(new Point((int)position.getX(), (int)(position.getY()+move)));
		}
		else if((k.getKeyCode()==KeyEvent.VK_LEFT || k.getKeyChar()=='a')&& (position.getX()-move>=0)) {
			if(world.isEmpty(new Point((int)(position.getX()-move)/tileSize, (int)(position.getY()/tileSize)), new Point(test, Settings.getCharacterHeight())))
			...
			if(world.isEmpty(new Point((int)(position.getX()+Settings.getCharacterWidth()+move)/tileSize, (int)(position.getY()/tileSize)), new Point(test, Settings.getCharacterHeight())))
				setPosition(new Point((int)position.getX()+move, (int)(position.getY())));
		}
	}
